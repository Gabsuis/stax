...
export default function Sidebar({ buildings, filteredBuildings, filterLabel }: Props) {
  ...
  const kpiBuildings = filteredBuildings ?? buildings
  const totalVacant = kpiBuildings.reduce((sum, b) => sum + b.vacantSqm, 0)
  const avgOcc = kpiBuildings.length
    ? Math.round((kpiBuildings.reduce((sum, b) => sum + b.occupancy, 0) / kpiBuildings.length) * 100)
    : 0
  ...
  return (
    <aside className="w-[220px] h-screen sticky top-0 shrink-0 border-s border-border bg-sidebar flex flex-col">
      ...
      <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto">
        {mainNav.map((item) => {
          const isAI = item.href === "/dashboard"
          ...
          return (
            <Link
              ...
              href={item.href}
              className={isAI
                ? `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all duration-200 text-lease-amber font-medium ${isActive ? "bg-lease-amber/10" : "hover:bg-primary/5"}`
                : navLinkClass(item.href)
              }
            >
              <item.icon className="w-4 h-4" strokeWidth={isActive || isAI ? 2 : 1.5} />
              {item.label}
            </Link>
          )
        })}
        ...
        <div className="pt-2 mt-2 border-t border-border/50">
          ...
          {crmOpen && (
            <div className="ms-4 mt-0.5">
              {crmItems.map((item) => (
                <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-3 py-1.5 rounded-lg text-sm transition-all duration-200 ${
                  pathname === item.href
                    ? "bg-primary/8 text-foreground font-medium"
                    : "text-muted-foreground hover:text-foreground hover:bg-primary/5"
                }`}>
        ...
        {/* Invoicing — always visible, independent of CRM */}
        <div className="pt-2 mt-2 border-t border-border/50">
      ...
      <div className="px-3 pb-4 space-y-2">
        {filterLabel && (
          <div className="text-xs font-medium text-foreground/70 px-1 truncate">{filterLabel}</div>
        )}
